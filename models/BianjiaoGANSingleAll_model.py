...
import cv2
from .WT_and_IWT import DWT, IWT
...
class BianjiaoGANSingleAllModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument('--lambda_wavelet_jiaohu', type=float, default=1.0, help='weight for H_loss')
    ...
    def __init__(self, opt):
        ...
        self.visual_names = ['real_A_center', 'fake_B_center','real_A_resize', 'fake_B_resize', 'fake_B_center_wavelet']
        ...
        if self.isTrain:
            self.model_names = ['G', 'G2', 'F', 'D']
        else:  # during test time, only load G
            self.model_names = ['G']

        # define networks (both generator and discriminator)
        self.netG = networks.define_G(opt.input_nc, opt.output_nc, opt.ngf, opt.netG, opt.normG, not opt.no_dropout, opt.init_type, opt.init_gain, opt.no_antialias, opt.no_antialias_up, self.gpu_ids, opt)
        self.netG2 = networks.define_G(opt.input_nc, opt.output_nc, opt.ngf, opt.netG, opt.normG, not opt.no_dropout, opt.init_type, opt.init_gain, opt.no_antialias, opt.no_antialias_up, self.gpu_ids, opt)
        ...
        if self.isTrain:
            ...
            self.optimizer_G2 = torch.optim.Adam(self.netG2.parameters(), lr=opt.lr, betas=(opt.beta1, opt.beta2))
    ...
    def data_dependent_initialize(self, data, iter):
        ...
        if self.opt.isTrain:
            ...
            self.compute_D_wavelet_loss().backward()                  # calculate gradients for D
    ...
    def optimize_parameters(self, iter):
        ...
        self.loss_D_wavelet = self.compute_D_wavelet_loss()
        self.loss_D = self.loss_D_high + self.loss_D_low + self.loss_D_wavelet
    ...
    def wavelet_transfer(self, input):
        LL, HL, LH, HH = self.DWT(input)
        LL_t, HL_t, LH_t, HH_t = self.netG(LL), self.netG2(HL), self.netG2(LH), self.netG2(HH)
        output = self.IWT(torch.stack((LL_t, HL_t, LH_t, HH_t), dim=1), self.device).squeeze(1)
        return output
    ...
    def forward(self,iter):
        ...
        self.fake_B_center_wavelet = self.wavelet_transfer(self.real_A_center)

    def center_crop(self, img, size=112):
        """裁剪中心区域"""
        h, w, _ = img.shape  # 获取图像尺寸
        start_x = (w - size) // 2  # 计算中心裁剪起点
        start_y = (h - size) // 2
        return img[start_y:start_y + size, start_x:start_x + size]  # 裁剪

    def resize_image(self, img, size=112):
        """调整图像大小"""
    ...
    def compute_D_wavelet_loss(self):
        """Calculate GAN loss for the discriminator"""
        fake = self.fake_B_center_wavelet.detach()
        # Fake; stop backprop to the generator by detaching fake_B
        pred_fake = self.netD(fake)
        self.loss_D_fake_wavelet = self.criterionGAN(pred_fake, False).mean()
        # Real
        self.pred_real = self.netD(self.real_B)
        loss_D_real = self.criterionGAN(self.pred_real, True)
        self.loss_D_real_wavelet = loss_D_real.mean()

        # combine loss and calculate gradients
        self.loss_D_wavelet = (self.loss_D_fake_wavelet + self.loss_D_real_wavelet) * 0.5
        return self.loss_D_wavelet

    def compute_G_loss(self, iter):
        """Calculate GAN and NCE loss for the generator"""
        fake_high = self.fake_B_center
        fake_low = self.fake_B_resize
        fake_wavelet = self.fake_B_center_wavelet

        # First, G(A) should fake the discriminator
        if self.opt.lambda_GAN > 0.0:
            pred_fake_high = self.netD(fake_high)
            pred_fake_low = self.netD(fake_low)
            pred_fake_wavelet = self.netD(fake_wavelet)
            self.loss_G_GAN = (self.criterionGAN(pred_fake_high, True).mean() + self.criterionGAN(pred_fake_low, True).mean() + self.criterionGAN(pred_fake_wavelet, True).mean()) * self.opt.lambda_GAN
        else:
            self.loss_G_GAN = 0.0
        ...
        if self.opt.lambda_wavelet_jiaohu > 0.0:
            self.loss_wavelet_jiaohu = self.L1_loss(self.fake_B_center, self.fake_B_center_wavelet)
        else:
            self.loss_wavelet_jiaohu = 0.0
        ...
        self.loss_G = self.loss_G_GAN + loss_NCE_both + self.loss_SR + self.loss_texture + self.loss_wavelet_jiaohu
        return self.loss_G
